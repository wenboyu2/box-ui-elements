...
class MultiputUpload extends BaseMultiput {
     ...
     * @param {string} [options.destinationFileId] - Untyped file id (e.g. no "f_" prefix)
    ...
    upload = ({
        ...
        if (id !== undefined && destinationFileId !== undefined) {
            throw new Error('Both id and destinationFileId set');
        }
        this.file = file;
        ...
        this.numPartsDigestComputing = 0; // # of parts sent to the digest sha1Worker
     ...
     * @return {void}
     */
    commitSessionErrorHandler = (error: Error): void => {
    ...
    canStartMorePartUploads = (): boolean =>
        !this.isDestroyed() && this.numPartsUploading < this.config.parallelism && this.numPartsDigestReady;
